...
- **01 The workshop** のラベルをクリックすると入口へ歩きます。
- 工房の入口付近で **E / Enter**、または右下の **Enter Room 1** でRoom 1へ移動します。
- Room 1の左上ホーム、右下Map、クリア画面の **Back to map** でMapへ戻ります。アニメーション実行中・音声処理中は遷移を待ちます。
- Room 1のクリア・単語・収集数をMapに表示します。保存形式は既存の `Hackathon.RoomOne.Progress.v1` を使い、入室時に進捗を消去しません。
