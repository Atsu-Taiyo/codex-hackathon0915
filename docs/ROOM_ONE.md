...
1. Watch the three-frame goal. Use **Watch goal** to replay it without revealing a sentence or granting progress.
...
4. Drag sentence cards onto one another to exchange positions. A selected slot also supports left/right arrow keys; Delete or double-click removes it.
5. Hover over a verb for its three-frame hint. Click **RUN** inside the builder, or press Enter, to run the sentence.
6. Open **Collection** for discovered scenes and **My experiments** for replayable history. **Map** shows Room 1 progress. Room 2 is an inactive future placeholder.
...
- All 24 syntactically valid combinations of two subjects, six verbs and two objects are accepted. The six robot-to-box actions and **The box lifts the robot.** use dedicated generated three-frame artwork. The reverse lift gives the box orange arms and legs; its hands raise and support the robot while its feet stay on the ground. Other unusual/self-directed combinations use a generic three-step actor/action/target illustration.
...
**Tools > Room One > Validate Rules** checks 24 legal meanings, malformed sentences, exact goal matching, collection/history behavior, JSON round trips and all art resources. **Tools > Room One > Build macOS MVP** builds `Builds/RoomOne.app`.
