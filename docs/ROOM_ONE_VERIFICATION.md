...
- 56 rule/resource checks: 12 supported meanings and 12 duplicate-card rejections, bad grammar and missing cards, exact semantic clear condition, reverse-lift routing, duplicate collection prevention, complete history, replay/preview exclusions, JSON round trips, five image resources and the transparent-sprite shader.
- 49 Play-mode checks: unified noun/verb placement, moving an existing card without duplication, replacement/removal returning cards, invalid drops, reset, duplicate execution rejection, discovery and persistence, card exchanges, all six standard actions, dedicated box-lifts-robot execution and saving, concurrent-run prevention, final-frame completion, first-clear overlay, unusual subject/object order, replay and saved progress. Reverse lifting completes without clearing the forward goal. Verification restores the saved PlayerPrefs value captured at the start of that run.
...
- UI rendering inspected: three goal images at top, reference-derived robot and orange box, same-size noun/verb cards, used-card empty positions, and a separate play/reset row inside the same white widget, vocabulary, map and collection links.
...
Latest card UI screenshot: `docs/RoomOne/CardWidget.png`. Explanatory sentences in the room are removed; discovery pulses and three-frame hints supply visual cues.
